function absorbsz1cm63A(filename)
%z is the constant value 1cm length of cuvette
...
for a = 1:length(v1a)
    v1c(a) = (-1/0.01)*log(abs(v1b(a))/abs(v2b(a)));
end
...
filename(txtdel:end) = [];
savename = strcat(filename, '-absorbnorm.txt');
...
ylabel('Intensity')
newtitle = strcat(filename, ' absorption normalized');
title(newtitle)
axis([340 1100 0 1])
