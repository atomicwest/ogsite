function absorb63Cz1cm(filename)
...
%open reference measurements
    fid2 = fopen('6.3B Milk-scattering.txt','r');
    v2 = fscanf(fid2, '%f');
    v2b = zeros((length(v2)),1); %reference intensity vector
    count2 = 1;
    numel(v2)
...
length(v1a)
for a = 1:2048
    v1c(a) = abs(-log(abs(v1b(a))/abs(v2b(a)))) ; %for alpha in cm
end
